...
exports.startMatchLoop = startMatchLoop;
 ...
 */
async function startMatchLoop(roomId, week) {
    // Update state to lock submissions
    db_1.db.run(`UPDATE rooms SET game_state = 'MATCH_DAY_LIVE' WHERE id = ?`, [roomId]);
    // Inform clients that UI should transition to LiveMatch overlay
    (0, socket_1.getIo)().to(roomId.toString()).emit('match_prep_started', { week });
    try {
        // 1. Fetch matches
        const matches = await new Promise((resolve, reject) => {
            db_1.db.all(`SELECT * FROM matches WHERE room_id = ? AND week = ?`, [roomId, week], (err, rows) => {
                if (err)
                    reject(err);
                else
                    resolve(rows);
        ...
        const liveMatchesData = await Promise.all(matches.map(async (m) => {
            ...
            // Play 1st Half
            const half1 = (0, simulation_1.simulateHalfTime)(homeStats, awayStats, 0, 45); // no referee bias for now
            return {
                matchId: m.id,
                homeStats,
                awayStats,
        ...
        // Finalize
        (0, socket_1.getIo)().to(roomId.toString()).emit('match_ended');
...
async function buildTeamStats(clubId, matchId, isHome) {
    ...
    try {
        ...
        if (ids.length > 0) {
            const pRows = await new Promise((res) => {
                db_1.db.all(`SELECT position, quality, aggressiveness, craque FROM players WHERE id IN (${ids.join(',')})`, (e, r) => res(r || []));
            });
            players = pRows;
        }
        else {
            // Fallback
            players = await new Promise((res) => {
                db_1.db.all(`SELECT position, quality, aggressiveness, craque FROM players WHERE club_id = ? LIMIT 11`, [clubId], (e, r) => res(r || []));
            });
        }
    }
    catch (e) {
...
async function broadcastHalf(roomId, liveMatches, startMin, endMin, objKey = 'half1') {
    for (let m = startMin; m <= endMin; m++) {
        // Collect events for minute `m` across all matches
        const minuteEvents = [];
        for (const match of liveMatches) {
            const matchHalf = match[objKey];
            const matchMinuteEvents = matchHalf.events.filter((e) => e.minute === m);
        ...
        (0, socket_1.getIo)().to(roomId.toString()).emit('match_tick', { minute: m, events: minuteEvents });
        // Wait 1 second = 1 game minute
        await new Promise(r => setTimeout(r, 1000));
    }
}
async function finalizeMatches(liveMatchesDataH2) {
    return new Promise((resolve) => {
        db_1.db.serialize(() => {
            db_1.db.run('BEGIN TRANSACTION');
            const stmt = db_1.db.prepare('UPDATE matches SET home_score=?, away_score=?, status=?, simulation_report=? WHERE id=?');
...
function getSingle(query, params) {
    return new Promise((resolve) => {
        db_1.db.get(query, params, (err, row) => resolve(row));
    });
}
