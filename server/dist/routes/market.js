...
const getDb = (query, params = []) => {
    return new Promise((resolve, reject) => {
        db_1.db.get(query, params, (err, row) => {
            if (err)
                reject(err);
            else
                resolve(row);
...
router.post('/list', async (req, res) => {
    ...
    if (!roomId || !playerId || !askingPrice || !type)
    ...
    try {
        ...
        db_1.db.run('INSERT INTO transfers (player_id, seller_club_id, asking_price, listing_type, auction_ends_at) VALUES (?, ?, ?, ?, ?)', [playerId, manager.club_id, askingPrice, type, endsAt], () => {
...
router.post('/bid', async (req, res) => {
    ...
    try {
        ...
        if (!transfer)
            return res.status(404).json({ error: 'Transfer not active' });
        if (transfer.listing_type !== 'AUCTION')
            return res.status(400).json({ error: 'This is not an auction' });
        if (transfer.seller_club_id === manager.club_id)
            return res.status(400).json({ error: 'Cannot bid on your own player' });
        const currentHighBid = Math.max(transfer.current_highest_bid, transfer.asking_price);
        if (bidAmount <= currentHighBid)
            return res.status(400).json({ error: 'Bid must be higher than ' + currentHighBid });
        // Check balance
        const club = await getDb('SELECT balance FROM clubs WHERE id = ?', [manager.club_id]);
        if (club.balance < bidAmount)
        ...
        db_1.db.run('UPDATE transfers SET current_highest_bid = ?, highest_bidder_club_id = ?, auction_ends_at = ? WHERE id = ?', [bidAmount, manager.club_id, newEndsAt, transferId], () => {
            (0, socket_1.getIo)().to(roomId.toString()).emit('market_update');
...
router.post('/buy', async (req, res) => {
    ...
    try {
        ...
        if (!transfer)
            return res.status(404).json({ error: 'Transfer not active' });
        if (transfer.listing_type !== 'LIST')
            return res.status(400).json({ error: 'This is not a direct buy' });
        if (transfer.seller_club_id === manager.club_id)
        ...
        db_1.db.serialize(() => {
            ...
            // Deduct buyer
            db_1.db.run('UPDATE clubs SET balance = balance - ? WHERE id = ?', [price, manager.club_id]);
            // Add seller
            db_1.db.run('UPDATE clubs SET balance = balance + ? WHERE id = ?', [price, transfer.seller_club_id]);
            // Transfer player
            db_1.db.run('UPDATE players SET club_id = ? WHERE id = ?', [manager.club_id, transfer.player_id]);
            // Mark as sold
            db_1.db.run('UPDATE transfers SET status = "SOLD" WHERE id = ?', [transferId]);
            db_1.db.run('COMMIT', () => {
                (0, socket_1.getIo)().to(roomId.toString()).emit('market_update');
