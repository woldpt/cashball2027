...
export async function startMatchLoop(roomId: number, week: number) {
  ...
  db.run(`UPDATE rooms SET game_state = 'MATCH_DAY_LIVE' WHERE id = ?`, [
    roomId,
  ]);
  ...
  try {
    ...
    const matches = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT * FROM matches WHERE room_id = ? AND week = ?`,
        [roomId, week],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
    ...
    await new Promise<void>((resolve) => {
      db.run(
        `UPDATE matches SET status = 'LIVE' WHERE id IN (${matchIds.join(",")})`,
        () => resolve(),
      );
    });
    ...
    const liveMatchesData = await Promise.all(
      matches.map(async (m) => {
    ...
    // Calculate 2nd Half
    const liveMatchesDataH2 = await Promise.all(
      liveMatchesData.map(async (m: any) => {
        const homeStats2 = await buildTeamStats(
          m.homeStats.isHome ? m.homeStats.clubId : m.awayStats.clubId,
          m.matchId,
          true,
        );
        const awayStats2 = await buildTeamStats(
          m.homeStats.isHome ? m.awayStats.clubId : m.homeStats.clubId,
          m.matchId,
          false,
        );
...
async function buildTeamStats(
  ...
  const tactic = await getSingle(
    `SELECT * FROM tactics WHERE club_id = ? AND match_id = ?`,
    [clubId, matchId],
  );
  ...
  try {
    ...
    if (ids.length > 0) {
      const pRows = await new Promise<any[]>((res) => {
        db.all(
          `SELECT position, quality, aggressiveness, craque FROM players WHERE id IN (${ids.join(",")})`,
          (e, r) => res(r || []),
        );
      });
      players = pRows;
      ...
      players = await new Promise<any[]>((res) => {
        db.all(
          `SELECT position, quality, aggressiveness, craque FROM players WHERE club_id = ? LIMIT 11`,
          [clubId],
          (e, r) => res(r || []),
        );
      });
...
async function broadcastHalf(
  ...
  for (let m = startMin; m <= endMin; m++) {
    ...
    for (const match of liveMatches) {
      ...
      const matchMinuteEvents = matchHalf.events.filter(
        (e: any) => e.minute === m,
      );
    ...
    getIo()
      .to(roomId.toString())
      .emit("match_tick", { minute: m, events: minuteEvents });
  ...
  return new Promise<void>((resolve) => {
    db.serialize(() => {
      ...
      const stmt = db.prepare(
        "UPDATE matches SET home_score=?, away_score=?, status=?, simulation_report=? WHERE id=?",
      );
...
function finishMatchLoop(roomId: number, week: number) {
  return new Promise<void>(async (resolve) => {
    ...
    db.run(
      `UPDATE rooms SET game_state = 'PRE_MATCH', current_week = ? WHERE id = ?`,
      [week + 1, roomId],
      () => {
        resolve();
      },
