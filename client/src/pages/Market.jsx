...
export default function Market() {
  ...
  const [roomId] = useState(localStorage.getItem("activeRoom"));
  ...
  const handleBuy = async (transferId, price) => {
    if (!confirm(`Comprar imediatamente por €${price.toLocaleString()}?`))
      return;
    try {
  ...
  const handleBid = async (transferId, minBid) => {
    const bidAmountStr = prompt(
      `Licitação Mínima Restante: €${minBid.toLocaleString()}.\nQual o valor que queres licitar?`,
      Math.floor(minBid * 1.05).toString(),
    );
      ...
      <div className="pb-4 border-b border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Todas as transações são registadas em tempo real.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {transfers.map((t) => (
            <div
              key={t.id}
              className="bg-[#161b22] border border-slate-800 rounded-xl p-5 hover:border-emerald-900 transition-colors flex justify-between items-center group relative overflow-hidden"
            >
              {/* Highlight flash if ends soon */}
              {t.listing_type === "AUCTION" &&
                t.auction_ends_at &&
                new Date(t.auction_ends_at).getTime() - Date.now() < 15000 && (
              ...
              <div className="z-10">
                <div className="flex items-center space-x-2 mb-1">
                  ...
                  <span className="bg-slate-800 text-slate-300 text-[10px] px-2 py-0.5 rounded font-bold tracking-wider">
                    {t.position}
                  </span>
                  ...
                  {t.craque === 1 && (
                    <span
                      className="inline-flex items-center justify-center bg-blue-500/10 text-blue-400 text-[10px] w-4 h-4 rounded-full"
                      title="Craque Mundial"
                    >
                      ★
                    </span>
                  )}
                ...
                {t.listing_type === "AUCTION" ? (
                  <button
                    onClick={() =>
                      handleBid(
                        t.id,
                        Math.max(t.current_highest_bid, t.asking_price),
                      )
                    }
